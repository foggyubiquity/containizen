...
select opt in "${options[@]}"; do
    ...
    curl -s "https://raw.githubusercontent.com/sotekton/containizen/master/languages/$opt-config.nix" 2>&1 | sed 's/\.\.\/config.nix/\.\/config.nix/' >example-config.nix
    curl -s "https://raw.githubusercontent.com/sotekton/containizen/master/config.nix" -o config.nix

    SOURCE_URL="docker://docker.io/sotekton/containizen:$opt"
    ...
    eval "$COMMON copy $SOURCE_URL '$DEST_URL:sotekton/containizen:$opt'"
done
